get_actions_girl_photos_sum1 = function (a = [], d = 3, g = 300, is_sunny = 1) {
	...
	if (d == 58) {
		...
		a.push({'desc':"RESET IF TYPHOON TOMORROW", 'imp':true});
	} else if (d == 31) {
	...
	if (flags['old_mus_box'] == 0 && is_sunny == 1) {
		if (d == 31) {
			...
			a.push({'desc':"Dig Music Box", 'cid':'f_old_mus_box', 'val':1, 'sr':true, 'sel':(flags['old_mus_box'] == 0 && !["SUN", "WED"].includes(dow))});
		}
	}
	...
	if (is_sunny != -1) { // No Typhoon
		...
		if (!is_festival(d)) {
			...
			if (aff[karen_id] < 100 && d < 41 && ["TUES", "WED", "FRI", "SAT"].includes(dow) && is_sunny == 1) {
				a.push({'desc':"Look for Karen in MTNS", 'iid':karen_id, 'imp':true});	
			}

			// MARIA in MTNS
			if (flags['photo_maria'] == 0 && is_sunny == 1 && (d <= 40 || dow == "MON")) {
			...
			if (dow != "THURS") {
				if (dow == "SUN" && is_sunny == 0 && aff[ann_id] >= _SICK_EVENT_MIN && flags['sick_ann'] == 0 && aff[ann_id] < _PHOTO_MIN) {
					...
					a.push({
						...
						'sel':((musbox_given || flags['new_mus_box'] == 0) && (d == 31 || !["WED", "SUN"].includes(dow))), 't2':" MusBox"
					});
					a.push({
						'desc':" MusBox", 'cid':[ann_id, 'f_new_mus_box'], 'val':[_MUS_BOX_AFF, -1], 'sr':true,
						'sel':(!musbox_given && (flags['new_mus_box'] == 1 && (d == 31 || !["WED", "SUN"].includes(dow)))), 't2':a[a.length - 1]['desc']
					});
					a.push({
			...
			if (dow == "MON" && is_sunny == 1) {
				...
				a.push({'desc':"Talk (Beach)", 'cid':elli_id, 'val':1, 't2':"MusBox ",
					'sel':(aff[elli_id] < _PHOTO_MIN), 'red':(aff[elli_id] >= _PHOTO_MIN)
				});
				...
				a.push({'desc':"Gift ", 'cid':elli_id, 'val':1, 'sr':true,
					't2':((vars['chickens'] > 0) ? "Egg " : "M/L Fish"), 'sel':(aff[elli_id] < _PHOTO_MIN && vars['chickens'] == 0)
				});
				a.push({'desc':"Egg ", 'sr':true, 't2':"Gift ", 'sel':(aff[elli_id] < _PHOTO_MIN && vars['chickens'] > 0),
			...
			if (is_sunny == 1) {
				if (dow == "THURS") {
					...
					// " Gift" <- -1 spaces
					// "Musbox  " <- 2 spaces
					a.push({'desc':"Talk (Rick Shop)", 'cid':ann_id, 'val':1, 'sel':(d == 60), 't2':"MusBox  "});
					a.push({'desc':"MusBox  ", 'cid':[ann_id, 'f_new_mus_box'], 'val':[_MUS_BOX_AFF, -1], 'sr':true, 'sel':false, 't2':a[a.length - 1]['desc']});
					a.push({'desc':" Gift", 'cid':ann_id, 'val':1, 'sr':true, 'sel':(d == 60)});
			...
			if (flags['photo_maria'] == 0) {
				if (is_sunny == 0 && dow == "MON" && aff[maria_id] < _PHOTO_MIN && aff[maria_id] >= _SICK_EVENT_MIN && flags['sick_maria'] == 0) {
					...
					a.push({'desc':"Talk (Library)", 'cid':maria_id, 'val':1, 't2':"MusBox",
						'sel':(!["SUN", "WED"].includes(dow) && (is_sunny == 0 || dow == "SAT")), 'red':(["SUN", "WED"].includes(dow))
					});
					...
					a.push({'desc':"Gift", 'cid':maria_id, 'val':2, 'sr':true, 'sel':(!["SUN", "WED"].includes(dow))});
				} else if (is_sunny == 0) {
					a.push({'desc':"Talk (Mayor House)", 'cid':maria_id, 'val':1, 't2':"MusBox"});
					a.push({'desc':"MusBox", 'cid':[maria_id, 'f_new_mus_box'], 'val':[_MUS_BOX_AFF, -1], 'sr':true,
				...
				if (!mar_sick) {
					if (flags['ankle_maria'] == 0 && aff[maria_id] >= _ANKLE_EVENT_MIN) {
						a.push({'desc':"ANKLE (Library)", 'cid':[maria_id, 'f_ankle_maria'], 'val':[_ANKLE_EVENT_AFF, 1],
							'sel':false, 'sr':(flags['dream_maria'] == 0),
							't2':["Talk", "Gift", a[a.length - 3]['desc']],
							'imp':(["WED", "SUN"].includes(dow))
						});
			...
			if (is_sunny == 1 && d < 60 && flags['corn_planted'] == 1) {
				// Water Corn
				a.push({'desc':"Equip Watering Can", 'imp':(!["SUN", "WED"].includes(dow))});
				a.push({'desc':("Water Corn" + ((vars['corn_waters'] >= _CORN_GROW_DAYS) ? "" : (" (" + (_CORN_GROW_DAYS - vars['corn_waters']) + " Left)"))),
					'cid':['v_watering_can_fill', 'v_corn_waters'], 'val':[-10, 1], 'sr':true, 'sel':(!["WED", "SUN"].includes(dow))
				});
			...
			if (d == 60 && aff[karen_id] < _DREAM_EVENT_AFF) {
				if (is_sunny == 1) {
					a.push({'desc':"Dog Karen to Pink (Vineyard)", 'cid':karen_id, 'val':(205 - aff[karen_id]), 'imp':true});
			...
			if (d == 31) { // Fireworks Festival
				...
				a.push({'desc':"Equip Watering Can"});
				a.push({'desc':("Water Corn (" + _CORN_GROW_DAYS + " Left)"), 'cid':['v_watering_can_fill', 'v_corn_waters'], 'val':[-10, 1], 'sr':true});
			...
			// Maria Photo
			if (flags['photo_maria'] == 0 && aff[maria_id] >= (_PHOTO_MIN - 2)) {
				// WARP TO PHOTO
				if (flags['dream_maria'] == 0) {
					a.push({'desc':"DREAM (Library)", 'cid':[maria_id, 'f_dream_maria'], 'val':[_DREAM_EVENT_AFF, 1],
						'sel':(!["SUN", "WED", "MON"].includes(dow)),
						'red':(["SUN", "WED", "MON"].includes(dow))
					});
				} else if (flags['dream_karen'] == 0 && aff[karen_id] > _DREAM_EVENT_MIN) {
					a.push({'desc':"DREAM (Vineyard)", 'cid':[karen_id, 'f_dream_karen'], 'val':[_DREAM_EVENT_AFF, 1],
						'sel':(!["SUN", "WED"].includes(dow)),
						'red':(["SUN", "WED"].includes(dow))
					});
				}
				a.push({'desc':"PHOTO at 6 PM", 'cid':[maria_id, 'f_photo_maria'], 'val':[_PHOTO_EVENT_AFF, 1],
					'sel':(!["SUN", "WED"].includes(dow)), 'imp':(d >= 58),
					'red':(["SUN", "WED"].includes(dow))
				});
			} else if (d == 60) {
				if (is_sunny == 1) {
					a.push({'desc':"Dog to Pink (Vineyard)", 'cid':karen_id, 'val':(205 - aff[karen_id]), 'imp':true});
				} else {
					a.push({'desc':"Sick Event", 'cid':[karen_id, 'f_sick_karen'], 'val':[_SICK_EVENT_AFF, 1], 'imp':true});
				}
				a.push({'desc':"Stay Up All Night", cid:'v_day', 'val':1, 'imp':true});
			}
...
visit_bar_sum=function(tmp_act = [], dow = get_dow(vars['day'], true), is_sunny = 1, imp_visit = true) {
	...
	if (dow != "SUN") {
		if (aff[karen_id] < 100) {
			if (vars['day'] < 41 && ["TUES", "WED", "FRI", "SAT"].includes(dow) && is_sunny == 1) {
				tmp_act.push({'desc':"Bring Dog to MTNS", 'iid':get_npc_id('dog')});
				tmp_act.push({'desc':"Dog to Yellow (MTNS)", 'cid':karen_id, 'val':(156 - aff[karen_id]), 'sel':false});
			} else if (["MON", "THURS", "FRI", "SAT"].includes(dow) && is_sunny == 1) {
				tmp_act.push({'desc':"Bring Dog to Bar", 'iid':get_npc_id('dog'), 'imp':true});
				tmp_act.push({'desc':"Dog to Yellow (BAR)", 'cid':karen_id, 'val':(156 - aff[karen_id]),
